...
JavaScript and CSS in `assets/dist` are generated from `admin-app` in the public release source: https://github.com/TWP-Technologies/sentient-forms-wp-plugin/tree/v0.3.15. Build with `cd admin-app`, `bun install --frozen-lockfile`, and `bun run build:wp`.
