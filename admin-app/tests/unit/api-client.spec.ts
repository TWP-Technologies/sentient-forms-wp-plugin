...
describe('SentientFormsApiClient', () => {
	...
	it('accepts a bodyless 204 response for a registered delete endpoint', async () => {
		mockFetch.mockResolvedValue(new Response(null, { status: 204 }));

		await expect(
			client.requestEndpoint(
				'forms.actions.delete',
				{ method: 'DELETE', showNotifications: false },
				'gravity_forms/forms/42/actions/mapping-1'
			)
	...
	it('rejects a missing required registered request body before network I/O', async () => {
		...
		try {
			await client.requestEndpoint('provider.openrouter.validate', {
				method: 'POST',
				showNotifications: false
			});
		} catch (error) {
		...
		expect(rejected).toMatchObject({ direction: 'request' });
		expect(String(rejected)).toContain('The request for local/providers/openrouter/validate');
	...
	it('accepts the production 200 deactivation response and invalidates license cache', async () => {
		mockFetch
			.mockResolvedValueOnce(jsonResponse(licenseInfoFixture('active')))
			.mockResolvedValueOnce(
				jsonResponse({
					success: true,
			...
			.mockResolvedValueOnce(jsonResponse(licenseInfoFixture('inactive')));
	...
	it('surfaces contaminated JSON responses without retaining raw response content', async () => {
		const secret = 'sk-live-must-not-escape';
		mockFetch.mockResolvedValue(
			new Response(`\uFEFF${secret}{"success":true,"data":{"saved":true}}`, {
				status: 200,
				headers: { 'content-type': 'application/json' }
			})
		);
		...
		let rejected: unknown;
		try {
			await client.getSettings({ showNotifications: false });
		} catch (error) {
			rejected = error;
		}
	...
	it('parses canonical managed checkout pending responses and rejects identity-less ones', async () => {
		const pending = {
			service: 'sentient-managed',
			activation_ready: false,
			status: 'pending',
			pending_reason: 'Waiting for Stripe.',
			site_url: 'https://example.test',
			local_site_identifier: 'example-local',
			billing_boundary: managedBillingBoundary
		};
		mockFetch.mockResolvedValueOnce(jsonResponse({ success: true, data: pending }));
		...
		mockFetch.mockResolvedValueOnce(
			jsonResponse({
				success: true,
				data: { activation_ready: false, status: 'pending', message: 'Waiting for Stripe.' }
			})
		);
		await expect(
			client.completeManagedCheckout(
	...
	it('parses local diagnostic settings and rejects retired remote telemetry payloads', async () => {
		mockFetch.mockResolvedValueOnce(
			jsonResponse({
				success: true,
				data: {
		...
		expect(JSON.parse(String(requestInit?.body))).toEqual({ local_diagnostics_enabled: true });

		mockFetch.mockResolvedValueOnce(
			jsonResponse({
				success: true,
				data: { telemetry_opt_in: true, synced_at: '2026-07-19T00:00:00Z' }
			})
		);
		await expect(client.getLocalDiagnosticsSettings({ showNotifications: false })).rejects.toThrow(
			ApiContractError
		);
	});
});
