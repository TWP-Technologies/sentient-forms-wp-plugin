...
const bypassCases = [
	...
	{
		name: 'storage reads through the global object',
		source: "const raw = globalThis.sessionStorage.getItem('secret');",
		rule: 'unparsed-storage-read'
	},
	{
		name: 'destructured legacy client requests',
		source: "const { request: load } = client;\nawait load('secrets');",
		rule: 'raw-network-call'
	},
	{
		name: 'bound JSON parse aliases',
		source: 'const parse = JSON.parse.bind(JSON);\nconst value = parse(raw);',
		rule: 'unparsed-json'
	},
	{
...
describe('Zod trust-boundary guard', () => {
	...
	it('isolates transport aliases between Svelte module and instance scripts', () => {
		const source = [
			'<script module lang="ts">',
			"  import { apiFetch as load } from '$lib/api/http';",
			'</script>',
			'<script lang="ts">',
			'  function load(): string { return "local"; }',
			'  const value = load();',
			'</script>'
		].join('\n');

		expect(findZodGuardViolations('src/routes/example/+page.svelte', source)).toEqual([]);
	});
