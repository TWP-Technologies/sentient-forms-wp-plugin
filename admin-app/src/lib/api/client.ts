...
export class ApiContractError extends Error {
	...
	constructor(
		...
		super(`The ${direction} for ${endpoint} did not match the ${schema} contract.`);
...
export class SentientFormsApiClient {
	...
	async getLocalDiagnosticsSettings(
		options: RequestOptions = {}
	): Promise<RegisteredEndpointResponse<'telemetry.read'>> {
	...
	async checkActionCompatibility(
		...
		const slug = formSourcePathSegment(formSourceSlug);
		const formIdSegment = formIdPathSegment(formId);
		const parsedRequest = parseRegisteredEndpointRequest('forms.actions.compatibility', request);
	...
	async getFormActionConfigs(
		...
		return this.unwrap<RegisteredEndpointResponse<'forms.actionConfigs.list'>>(response).configs;
	}
	...
	async getFormActionConfig(
		...
		return this.unwrap<RegisteredEndpointResponse<'forms.actionConfigs.read'>>(response).config;
	}
	...
	async getActionDefaultsBatch(
		...
		for (let index = 0; index < ids.length; index += ACTION_DEFAULTS_BATCH_LIMIT) {
			...
			Object.assign(
				defaults,
				this.unwrap<RegisteredEndpointResponse<'actions.defaults.batch'>>(response).defaults ?? {}
			);
		}
		return defaults;
	}
	...
	private validateFormActionConfigPayload(config: Partial<FormActionConfig>) {
	...
	async requestEndpoint<TName extends EndpointName>(
		...
		const responseSchema = z.preprocess((payload) => {
			if (
				payload !== null &&
