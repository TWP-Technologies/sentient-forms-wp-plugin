...
function phpNumericKeyMap<TSchema extends z.ZodType>(schema: TSchema) {
	return z.preprocess(
		(payload) =>
			Array.isArray(payload)
				? Object.fromEntries(payload.map((value, index) => [String(index), value]))
				: payload,
		schema
	);
}
...
const localDiagnosticsUpdateRequestSchema = z.strictObject({
...
const localCustomActionRecordSchema = z.object({
	...
	code: nullableTextSchema,
	display_name: nullableTextSchema,
	definition_json: phpMap(jsonRecordValueSchema).nullable().nonoptional(),
...
const localFormMappingRecordSchema = z.object({
	...
	input_bindings_json: phpMap(jsonRecordValueSchema).nullable().nonoptional(),
...
const leadHistoricalRunSchema = z.object({
	...
	filters: phpMap(jsonRecordValueSchema),
...
const realtimeSettingsBoundarySchema = z.object(realtimeSettingsBoundaryShape);
const realtimeSettingsRequestBoundarySchema = z.strictObject(realtimeSettingsBoundaryShape);

const mappingIncludeSiteContextBoundarySchema = z.preprocess(
	(value) => {
		if (value === 'yes') return 'always';
		if (value === 'no') return 'never';
		return value;
	},
	z.enum(['global', 'always', 'never'])
);
const formActionConfigBoundaryShape = {
...
const canonicalInputMappingBoundaryShape = {
	mode: z.enum(['all', 'selected', 'exclude']),
	field_ids: z.array(z.string()).optional(),
	include_metadata: z.boolean().optional()
};
const canonicalInputMappingBoundarySchema = z.object(canonicalInputMappingBoundaryShape);
const canonicalInputMappingRequestBoundarySchema = z.strictObject(
	canonicalInputMappingBoundaryShape
);
...
const formActionSettingsBoundaryShape = {
	input_mapping: z
	...
	suppress_webhooks_on_spam: z.boolean().optional(),
	skip_downstream_on_spam: z.boolean().optional(),
	spam_confidence_threshold: z.number().min(0).max(1).optional(),
	spam_result_display_mode: z.string().optional(),
	spam_indicators_display: z.string().optional(),
	spam_positive_examples: z.array(spamGuidanceExampleBoundarySchema).optional(),
	spam_negative_examples: z.array(spamGuidanceExampleBoundarySchema).optional(),
...
const formActionSettingsBoundarySchema = phpMap(
	z.object(formActionSettingsBoundaryShape).catchall(z.json())
);

const strictSpamGuidanceExampleSourceBoundarySchema =
	spamGuidanceExampleSourceBoundarySchema.strict();
const strictSpamGuidanceExampleBoundarySchema = spamGuidanceExampleBoundarySchema
	.extend({ source: strictSpamGuidanceExampleSourceBoundarySchema.optional() })
	.strict();
const strictTriggerSourceBoundarySchema = z.strictObject({
	type: z.enum(['hook_root', 'mapping']),
	mapping_id: z.string().optional()
});
const strictAttachmentMappingBoundarySchema = z.strictObject({
	mode: z.enum(['none', 'gf_upload', 'media_library', 'mixed']),
	gf_upload_field_ids: z.array(z.string()).optional(),
	media_ids: z.array(z.number().int()).optional(),
	max_files: z.number().int().optional()
});
const strictBatchSettingsBoundarySchema = z.strictObject({
	enabled: z.boolean(),
	delay_seconds: z.number(),
	max_wait_seconds: z.number()
});
const strictConditionRuleBoundarySchema = conditionRuleBoundarySchema.strict();
const strictConditionNodeBoundarySchema: z.ZodType<ConditionNodeBoundary> = z.lazy(() =>
	z.union([
		strictConditionRuleBoundarySchema,
		z.strictObject({
			type: z.literal('group'),
			logic: z.enum(['all', 'any']),
			rules: z.array(strictConditionNodeBoundarySchema)
		})
	])
);
const strictConditionsBoundarySchema = z.strictObject({
	enabled: z.boolean(),
	root: z.strictObject({
		type: z.literal('group'),
		logic: z.enum(['all', 'any']),
...
const strictPostExecutionActionBoundarySchema = postExecutionActionBoundarySchema.strict();
const formActionSettingsRequestBoundarySchema = z.strictObject({
	local_form_mapping_id: z.number().int().positive().optional(),
	effect_mapping_json: nullableJsonRecordValueSchema.optional(),
	input_mapping: canonicalInputMappingRequestBoundarySchema.optional(),
	attachment_mapping: strictAttachmentMappingBoundarySchema.optional(),
	dependency_ids: z.array(z.string()).optional(),
	trigger_sources: z.record(z.string(), strictTriggerSourceBoundarySchema).optional(),
	skip_on_upstream_spam: z.boolean().optional(),
	suppress_notifications_on_spam: z.boolean().optional(),
	suppress_webhooks_on_spam: z.boolean().optional(),
	skip_downstream_on_spam: z.boolean().optional(),
	...
	spam_result_display_mode: z.string().optional(),
	spam_indicators_display: z.string().optional(),
	spam_positive_examples: z.array(strictSpamGuidanceExampleBoundarySchema).optional(),
	...
	prompt_overrides: jsonRecordValueSchema.optional(),
	post_execution_actions: z.array(strictPostExecutionActionBoundarySchema).optional(),
	execution_mode: z.enum(['validation', 'after_submission', 'real_time']).optional(),
	async: z.boolean().optional(),
	realtime_settings: realtimeSettingsRequestBoundarySchema.optional(),
	batch_settings: strictBatchSettingsBoundarySchema.optional(),
	linked_action_status: z.string().optional(),
	repair_state: z.string().optional()
});
...
const formActionMutationRequestSchema = z.strictObject({
	central_action_id: z.string().optional(),
	action_type_indicator: z.enum(['master', 'custom', 'local_first']).optional(),
	trigger_hooks: z.array(z.string()).optional(),
	is_action_enabled_for_form: z.boolean().optional(),
	execution_priority: z.number().int().optional(),
	action_name_label: z.string().optional(),
	settings: formActionSettingsRequestBoundarySchema.optional()
});
...
const actionDefinitionBoundarySchema = z
	.object({
		id: z.string(),
		...
		base_credit_cost: z.number().nullable().optional(),
		...
		override_schema: z.record(z.string(), overrideKeySchemaBoundary).optional(),
		promptTemplate: nullableTextSchema.optional(),
		prompt_template: nullableTextSchema.optional(),
		structuredOutputSchema: nullableJsonRecordValueSchema.optional(),
		structured_output_schema: nullableJsonRecordValueSchema.optional(),
	...
	.transform(
		({
			...
			...definition,
			templateId: definition.templateId ?? template_id,
			baseCreditCost: definition.baseCreditCost ?? base_credit_cost,
			modelHint: definition.modelHint ?? model_hint,
			overrideSchema: definition.overrideSchema ?? override_schema,
			promptTemplate: definition.promptTemplate ?? prompt_template,
			structuredOutputSchema: definition.structuredOutputSchema ?? structured_output_schema
		})
	);
const customActionBoundarySchema = z.object({
	...
	display_name: z.string(),
	description: nullableTextSchema,
	prompt_overrides: phpMap(jsonRecordValueSchema),
...
export const formSourceDescriptorBoundarySchema = z.object({
	...
	capabilities: z
		...
		.optional(),
	lifecycles: phpMap(z.record(z.string(), lifecycleDescriptorBoundarySchema)),
	native_entry: z
...
const capabilitiesBoundarySchema = z.object({
	...
	cps_version: z.string().nullable().optional()
});
...
const submissionLedgerRecordBoundarySchema = z.object({
	...
	logical_fields: phpMap(jsonRecordValueSchema),
...
const formActionsBootstrapBoundarySchema = z.object({
	...
	form_action_configs: phpMap(z.record(z.string(), formActionConfigBoundarySchema)).optional(),
	form_fields: z.array(formFieldBoundarySchema).optional(),
	action_defaults: phpMap(z.record(z.string(), formActionConfigBoundarySchema)).optional(),
...
const requestTraceBoundarySchema = z.object({
	...
	input: z.object({
		...
		values: phpMap(z.record(z.string(), z.string())),
...
const allFormActionConfigsBoundarySchema = z.object({
	form_source: z.string(),
	form_id: z.union([z.string(), z.number()]),
	configs: phpMap(z.record(z.string(), formActionConfigBoundarySchema))
});
const actionDefaultsBatchBoundarySchema = z.object({
	defaults: phpMap(z.record(z.string(), formActionConfigBoundarySchema)),
	generated_at: z.string().optional()
});
...
const workflowMeteringBoundarySchema = z.object({
	...
	credits_by_node: phpNumericKeyMap(z.record(z.string(), z.number())),
...
export const endpointRegistry = {
	...
	'billing.managedCheckout.complete': {
		...
		response: managedCheckoutCompleteResponseSchema.describe(
			'billing.managedCheckout.complete response'
		),
		error: endpointErrorSchema.describe('billing.managedCheckout.complete error')
	},
	'telemetry.read': {
		path: 'telemetry',
		response: localDiagnosticsSettingsResponseSchema.describe('telemetry.read response'),
		error: endpointErrorSchema.describe('telemetry.read error')
	},
	'telemetry.update': {
		path: 'telemetry',
		request: localDiagnosticsUpdateRequestSchema.describe('telemetry.update request'),
		response: localDiagnosticsSettingsResponseSchema.describe('telemetry.update response'),
	...
	'forms.actionConfigs.update': {
		path: '{source}/forms/{formId}/action-config/{actionId}',
		request: formActionConfigRequestBoundarySchema.describe('forms.actionConfigs.update request'),
	...
	'billing.checkout.create': {
		...
		response: billingCheckoutSessionResponseSchema.describe('billing.checkout.create response'),
	...
	'billing.managedCheckout.start': {
		...
		response: managedCheckoutStartResponseSchema.describe('billing.managedCheckout.start response'),
	...
	'siteContext.withdraw': {
		path: 'site-context',
		request: emptyRequestSchema.describe('siteContext.withdraw request'),
		response: rawOrSuccessEnvelope(siteContextBoundaryResponseSchema).describe(
			'siteContext.withdraw response'
		),
		error: endpointErrorSchema.describe('siteContext.withdraw error')
	},
...
export type RegisteredEndpointRequest<TName extends EndpointName> =
	(typeof endpointRegistry)[TName] extends { request: infer TRequest extends z.ZodType }
		? z.input<TRequest>
		: never;
export type ParsedEndpointRequest<TName extends EndpointName> =
	(typeof endpointRegistry)[TName] extends { request: infer TRequest extends z.ZodType }
		? z.output<TRequest>
		: never;
...
export function parseRegisteredEndpointRequest<TName extends EndpointName>(
	...
	return definition.request.parse(payload) as ParsedEndpointRequest<TName>;
}
