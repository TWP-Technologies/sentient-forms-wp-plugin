import { z } from 'zod';
import { formSourceDescriptorBoundarySchema, tierValueSchema } from '$lib/api/endpoint-schemas';
...
export function readRuntimeConfigSafely(): SentientFormsConfig | undefined {
	try {
		...
		if (error instanceof RuntimeConfigError) {
			console.warn('[SentientForms] Rejected runtime config.', { issues: error.issues });
		} else {
			console.warn('[SentientForms] Runtime config could not be read.');
		}
		return undefined;
	}
}
