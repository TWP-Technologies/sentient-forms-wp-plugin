<script lang="ts">
	...
	const state = customActionsState;

	let statusFilter: 'active' | 'archived' | 'all' = (state.filters.status as 'active' | 'archived') ?? 'active';
...
let templateId = '';
	...
	onMount(() => {
		customActionsStore.load(state.filters);
	});
...
>
	<div slot="actions" class="sf-flex sf-gap-2 sf-flex-wrap">
		<Button variant="secondary" onclick={() => customActionsStore.reload()} disabled={state.loading}>
			Refresh
		</Button>
	</div>

	{#if state.error}
		<Alert variant="danger" class="sf-mb-4">{state.error}</Alert>
	{/if}

		{#if state.quota}
			<Card class="sf-mb-4">
				<div class="sf-flex sf-items-center sf-justify-between sf-gap-4 sf-flex-col md:sf-flex-row">
					<div>
						<p class="sf-text-sm sf-text-slate-500">Quota usage</p>
						<p class="sf-text-xl sf-font-semibold">
							{state.quota.quota_used} / {state.quota.quota_max}
						</p>
						<p class="sf-text-xs sf-text-slate-500">
							{state.quota.quota_remaining} remaining actions
						</p>
					</div>
					<div class="sf-w-full md:sf-w-1/2">
						<div class="sf-h-2 sf-rounded-full sf-bg-slate-200">
							<div
								class="sf-h-2 sf-rounded-full sf-bg-indigo-500"
								style={`width: ${Math.min(
									100,
									(state.quota.quota_used / Math.max(1, state.quota.quota_max)) * 100
								)}%`}
							></div>
						</div>
					</div>
	...
	<Card class="sf-mb-6">
		<div class="sf-flex sf-flex-wrap sf-gap-2 sf-items-center sf-justify-between">
			...
			<span class="sf-text-xs sf-text-slate-500">
				Last synced:
				{state.lastLoadedAt ? new Date(state.lastLoadedAt).toLocaleTimeString() : 'never'}
			</span>
		</div>
	...
	<Card>
		...
		<form class="sf-flex sf-flex-col sf-gap-4" data-testid="custom-action-form" onsubmit={handleCreate}>
			...
			/>
			<div class="sf-grid sf-gap-4 md:sf-grid-cols-2">
				<InputField
			...
			<div class="sf-flex sf-justify-end">
				<Button type="submit" disabled={state.creating}>
					{state.creating ? 'Creating…' : 'Create action'}
				</Button>
			</div>
		</form>
	</Card>
