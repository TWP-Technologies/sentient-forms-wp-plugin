...
- Run `bun run svelte:guard` (part of `bun run qa:full`) before opening a PR; it executes the lockfile-installed `svelte-check` and fails if legacy syntax or `createEventDispatcher` usage slips back in. Required QA must not resolve an unpinned `sv` package at runtime.
