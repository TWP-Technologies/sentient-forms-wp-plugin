...
`Sentient_Forms_Accepted_Submission_Adapter_Interface` is the optional accepted-submission lifecycle contract. Adapters declare the source-native accepted hook and normalize native submission data; the lifecycle-neutral `Sentient_Forms_Form_Source_Workflow_Runner` owns ledger capture and linkage, workflow configuration, mapping planning, conditions, dependency and idempotency context, and async scheduling without branching on Form Source identifiers. Contact Form 7 is the first migrated adapter and continues to enter this lifecycle only from `wpcf7_mail_sent`, after mail has been sent successfully. Earlier CF7 hooks do not enter the shared runner. Validation will enter this same runner through its own thin lifecycle method rather than a sibling validation orchestrator.
