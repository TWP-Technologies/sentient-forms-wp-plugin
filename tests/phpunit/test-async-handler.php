...
class AsyncHandlerTest extends WP_UnitTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // Record outbound requests; focused tests provide their own provider responses.
        add_filter(
            'pre_http_request',
    ...
    public function test_dispatch_action_evaluation_emits_duplicate_block_event(): void
    {
        ...
        $this->assertSame( 'duplicate_blocked', $first['metadata']['reason'] ?? null );
    }

    public function test_debug_mode_does_not_bypass_local_diagnostic_consent(): void
    {
        $this->plugin->set_telemetry_settings( [ 'telemetry_opt_in' => false ] );
        $this->plugin->update_options( [ 'global_settings' => [ 'debug_mode' => true ] ] );
        ...
        $handler = $this->plugin->get_async_handler();
        $method  = new ReflectionMethod( $handler, 'emit_async_event' );
        $method->invoke(
            $handler,
            'local_mapping_failed',
            [
                'action_code'          => 'entry_summary_v1',
                'execution_request_id' => 'req-safe-metadata',
                'provider'             => 'sentient_managed',
                'settings'             => [ 'prompt' => 'private mapping prompt' ],
                'form'                 => [ 'email' => 'visitor@example.test' ],
                'entry'                => [ 'message' => 'private submission' ],
            ...
            [
                ...
                'error'    => 'raw provider failure with private content',
    ...
    public function test_terminal_failure_emits_only_stable_error_code_after_consent(): void
    {
        $this->plugin->update_options( [ 'global_settings' => [ 'debug_mode' => true ] ] );
        $this->plugin->set_telemetry_settings( [ 'telemetry_opt_in' => true ] );
        $debug_events = [];
        $async_events = [];
        $debug_listener = static function ( string $message, array $context ) use ( &$debug_events ): void {
            $debug_events[] = compact( 'message', 'context' );
        };
        $async_listener = static function ( array $event ) use ( &$async_events ): void {
            $async_events[] = $event;
        };
        add_action( 'sentient_forms_debug_log', $debug_listener, 10, 2 );
        add_action( 'sentient_forms_async_event', $async_listener, 10, 1 );

        try
        {
            $method = new ReflectionMethod( $this->plugin->get_async_handler(), 'handle_evaluation_failure' );
            $method->invoke(
                $this->plugin->get_async_handler(),
                [
                    'attempt'      => 1,
                    'max_attempts' => 1,
                    'action_code'  => 'entry_summary_v1',
                ...
                [],
                new WP_Error( 'provider_secret_failure', 'raw provider failure sk-or-private-secret visitor@example.test' )
            );
        }
        finally
        {
            remove_action( 'sentient_forms_debug_log', $debug_listener, 10 );
            remove_action( 'sentient_forms_async_event', $async_listener, 10 );
        }

        $this->assertSame( [], $debug_events );
        $this->assertCount( 1, $async_events );
        $this->assertSame( 'provider_secret_failure', $async_events[0]['metadata']['error_code'] ?? null );
        $encoded = wp_json_encode( $async_events );
        $this->assertStringNotContainsString( 'raw provider failure', $encoded );
        $this->assertStringNotContainsString( 'sk-or-private-secret', $encoded );
        $this->assertStringNotContainsString( 'visitor@example.test', $encoded );
    }

    public function test_enabled_local_logger_records_only_allowlisted_diagnostic_metadata(): void
    {
        $this->plugin->set_telemetry_settings( [ 'telemetry_opt_in' => true ] );
        $enable_logging = static fn(): bool => true;
        add_filter( 'sentient_forms_enable_logging', $enable_logging );

        $logger_property = new ReflectionProperty( $this->plugin, 'logger' );
        $logger_property->setValue( $this->plugin, null );
        $handler = new Sentient_Forms_Async_Handler( $this->plugin );
        $log_path = $this->plugin->get_logger()->get_log_path();
        ...
        try
        {
            ...
            $method->invoke(
                ...
                [
                    'action_code'          => 'entry_summary_v1',
                    'execution_request_id' => 'req-safe-local-log',
                    'provider'             => 'sentient_managed',
                    'entry'                => [ 'email' => 'visitor@example.test' ],
                    'api_key'              => 'sk-or-private-secret',
                ],
                [
                    'error_code' => 'provider_secret_failure',
                    'reason'     => 'raw provider reason visitor@example.test',
                    'error'      => 'raw provider failure visitor@example.test',
                    'result'     => [ 'output' => 'private model output' ],
                ]
            );
            ...
            $this->assertStringContainsString( 'local_mapping_failed', $contents );
            $this->assertStringContainsString( 'provider_secret_failure', $contents );
            $this->assertStringContainsString( 'req-safe-local-log', $contents );
            $this->assertStringNotContainsString( 'raw provider failure', $contents );
            $this->assertStringNotContainsString( 'raw provider reason', $contents );
            $this->assertStringNotContainsString( 'visitor@example.test', $contents );
            $this->assertStringNotContainsString( 'sk-or-private-secret', $contents );
            $this->assertStringNotContainsString( 'private model output', $contents );
        }
        finally
        {
            remove_action( 'sentient_forms_async_event', [ $handler, 'write_local_diagnostic_event' ], 10 );
            remove_action( 'sentient_forms_evaluate_action', [ $handler, 'process_evaluation' ], 10 );
            remove_action( 'sentient_forms_process_local_mapping', [ $handler, 'process_local_mapping' ], 10 );
            remove_filter( 'sentient_forms_enable_logging', $enable_logging );
            $logger_property->setValue( $this->plugin, null );
            if ( file_exists( $log_path ) )
            {
                wp_delete_file( $log_path );
    ...
    public function test_default_local_diagnostic_writer_rechecks_consent_for_public_hook_calls(): void
    {
        $this->plugin->set_telemetry_settings( [ 'telemetry_opt_in' => false ] );
        $enable_logging = static fn(): bool => true;
        add_filter( 'sentient_forms_enable_logging', $enable_logging );
        ...
        try
        {
            $handler->write_local_diagnostic_event(
                [
                    'event'    => 'hostile_public_hook_call',
            ...
            $this->assertFileDoesNotExist(
                $log_path,
                'The default writer must not trust that public hook callers already passed the consent gate.'
            );
        }
        finally
        {
    ...
    public function test_generic_debug_log_requires_consent_and_redacts_hostile_callers(): void
    {
        $original_plugin_settings = get_option( 'sentient_forms_plugin_settings', [] );
        update_option( 'sentient_forms_plugin_settings', [ 'enable_logging' => true ] );
        $this->plugin->set_telemetry_settings( [ 'telemetry_opt_in' => false ] );
        $events = [];
        $listener = static function ( string $message, array $context ) use ( &$events ): void {
            $events[] = compact( 'message', 'context' );
        };
        add_action( 'sentient_forms_debug_log', $listener, 10, 2 );

        try
        {
            sentient_forms_debug_log(
                'raw provider failure sk-or-private-secret visitor@example.test',
                [
                    'diagnostic_code'       => 'provider_request_failed',
                    'error_code'            => 'provider_timeout',
                    'reason'                => 'raw provider reason visitor@example.test',
                    'action_code'           => 'entry_summary_v1',
                    'execution_request_id'  => 'req-safe-debug-log',
                    'entry'                 => [ 'email' => 'visitor@example.test' ],
                    'raw_response'          => 'private model output',
                ]
            );
            $this->assertSame( [], $events, 'Debug mode or local logging must not bypass diagnostic consent.' );
            ...
            $this->assertSame( 'Sentient Forms diagnostic event.', $events[0]['message'] );
            $this->assertSame( 'provider_request_failed', $events[0]['context']['diagnostic_code'] ?? null );
            $this->assertSame( 'provider_timeout', $events[0]['context']['error_code'] ?? null );
            $this->assertSame( 'entry_summary_v1', $events[0]['context']['action_code'] ?? null );
            $this->assertSame( 'req-safe-debug-log', $events[0]['context']['execution_request_id'] ?? null );
            $encoded = wp_json_encode( $events );
            $this->assertStringNotContainsString( 'raw provider failure', $encoded );
            $this->assertStringNotContainsString( 'raw provider reason', $encoded );
            $this->assertStringNotContainsString( 'sk-or-private-secret', $encoded );
            $this->assertStringNotContainsString( 'visitor@example.test', $encoded );
            $this->assertStringNotContainsString( 'private model output', $encoded );
        }
        finally
        {
            remove_action( 'sentient_forms_debug_log', $listener, 10 );
            update_option( 'sentient_forms_plugin_settings', $original_plugin_settings );
        }
    }

    public function test_stale_legacy_telemetry_rows_are_retired_without_false_delivery_claim(): void
    {
        $store = $this->plugin->get_async_request_store();
        $hash  = $store->enqueue_telemetry(
            'async_job_success',
            [ 'action_code' => 'entry_summary_v1' ]
        );

        $handler = $this->plugin->get_async_handler();
        $method  = new ReflectionMethod( $handler, 'sweep_stale_async_rows' );
        $method->invoke( $handler );

        $row = $store->get( $hash, 'telemetry' );
        $this->assertIsArray( $row );
        $this->assertSame( 'telemetry_retired', $row['status'] ?? null );
        $this->assertStringContainsString( 'retired', strtolower( (string) ( $row['last_error'] ?? '' ) ) );
        $this->assertNotSame( 'telemetry_sent', $row['status'] ?? null );
    }
    ...
    public function test_process_local_mapping_executes_elementor_mapping_from_submission_ledger_identifiers(): void
    {
        ...
        $scheduled = $handler->schedule_local_mapping(
            ...
            [
                ...
                'central_action_id'    => 'elementor_local_async_summary',
    ...
    public function test_process_local_mapping_retries_transient_openrouter_failure(): void
    {
        ...
        $scheduled = $handler->schedule_local_mapping(
            ...
            [
                ...
                'entry_id'             => 655,
                'action_id'            => 'local_first_' . $mapping_id,
                'central_action_id'    => 'local_async_retry_summary',
    ...
    public function test_process_local_mapping_does_not_retry_missing_auth_transport_failure(): void
    {
        ...
        $scheduled = $handler->schedule_local_mapping(
            ...
            [
                ...
                'entry_id'             => 656,
                'action_id'            => 'local_first_' . $mapping_id,
                'central_action_id'    => 'local_async_terminal_summary',
