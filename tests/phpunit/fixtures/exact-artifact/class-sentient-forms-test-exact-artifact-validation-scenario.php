...
final class Sentient_Forms_Test_Exact_Artifact_Validation_Scenario
{
    ...
    public static function run( string $form_source, string $action_code, string $assignment ): array
    {
        ...
        $trace = self::trace_from_headers( $headers );
        ...
        if ( 'reject' === $assignment && ( ! $rejected || ! is_string( $request_id ) || '' === $request_id || ! is_string( $trace_id ) || '' === $trace_id ) )
        {
            throw new RuntimeException(
                sprintf(
                    'Rejected validation evidence is incomplete for %s/%s: rejected=%s request=%s trace=%s.',
                    $form_source,
                    $action_code,
                    $rejected ? 'true' : 'false',
    ...
    ): array
    {
        ...
        $rejected = $spam || [] !== $result->invalidations;
    ...
    ): array
    {
        ...
        $adapter = new Sentient_Forms_WPForms_Adapter( Sentient_Forms_Plugin::instance(), $runner, $emitter );
        $adapter->init();
        $fields = [
    ...
    ): array
    {
        ...
        $adapter = new Sentient_Forms_Elementor_Forms_Adapter( Sentient_Forms_Plugin::instance(), $runner, $emitter );
        $adapter->init();
    ...
    /** @param array<int, array{0: string, 1: string}> $headers @return array<string, string>|array{} */
    private static function trace_from_headers( array $headers ): array
    {
        foreach ( $headers as [ $name, $value ] )
        {
            if ( 'X-Sentient-Forms-Validation-Trace' !== $name )
            {
                continue;
            }
            $payload = json_decode( rawurldecode( $value ), true );
            $trace = is_array( $payload ) ? ( $payload['rejections'][0] ?? null ) : null;
            if ( is_array( $trace ) )
            {
                return $trace;
            }
