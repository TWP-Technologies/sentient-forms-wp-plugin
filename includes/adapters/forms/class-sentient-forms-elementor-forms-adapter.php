...
class Sentient_Forms_Elementor_Forms_Adapter implements Sentient_Forms_Adapter_Interface, Sentient_Forms_Async_Capable_Adapter_Interface, Sentient_Forms_Historical_Entries_Adapter_Interface, Sentient_Forms_Accepted_Submission_Adapter_Interface, Sentient_Forms_Validation_Adapter_Interface, Sentient_Forms_Native_Validation_Effects_Adapter_Interface
{
    ...
    {
        return Sentient_Forms_Form_Sources::ELEMENTOR_PRO_FORMS;
    }

    public function get_name(): string
    {
        return __( 'Elementor Pro Forms', 'sentient-forms' );
    }
    ...
    public function get_capability_descriptor(): array
    {
        ...
        if ( ! $has_elementor )
        {
            ...
            $message      = __( 'Install and activate Elementor Pro to configure Sentient Forms for Elementor Pro Forms.', 'sentient-forms' );
            $reason       = __( 'Elementor is not active on this site.', 'sentient-forms' );
        }
        elseif ( ! $has_pro_forms )
        {
            $availability = 'requires_pro';
            $message      = __( 'Elementor Pro Forms support requires Elementor Pro Forms APIs.', 'sentient-forms' );
        ...
        return [
            ...
            'lifecycles'           => [
                ...
                Sentient_Forms_Form_Source_Lifecycles::REAL_TIME        => [
                    ...
                    'requires_ledger'    => false,
                    'unsupported_reason' => __( 'Realtime Elementor Pro Forms support is not available in this release.', 'sentient-forms' ),
                ],
            ],
            'native_entry'         => [
                'id'    => false,
            ...
            'ledger'               => [
                ...
                'unavailable_reason'  => __( 'Enable the Sentient Forms Submission Ledger before reviewing Elementor Pro Forms submissions in Sentient Forms.', 'sentient-forms' ),
            ],
            'requirements'         => [
    ...
    {
        if ( ! is_array( $native_submission ) || ! array_key_exists( 'record', $native_submission ) )
        {
            return new WP_Error( 'sentient_forms_elementor_invalid_accepted_submission', __( 'Elementor accepted submission payload is invalid.', 'sentient-forms' ) );
        }

        $record  = $native_submission['record'];
        $handler = $native_submission['handler'] ?? null;
        $form_id = $this->resolve_form_id_from_record( $record, $handler );
        if ( '' === $form_id )
        {
            $this->emit_form_resolution_failure( $record );

            return new WP_Error( 'sentient_forms_elementor_form_resolution_failed', __( 'Elementor accepted submission could not be matched to a form.', 'sentient-forms' ) );
        }
    ...
    public function update_form_settings( mixed $form_id, array $settings ): bool | WP_Error
    {
        return Sentient_Forms_Legacy_Action_Authority_Migrator::update_action_option(
            $this->get_form_actions_option_key( $form_id ),
            $settings
        );
    }
    ...
    private function form_snapshot( string $form_id ): array
    {
        ...
        return [
            ...
            'title'       => is_array( $form ) ? $this->form_title( $form ) : sprintf(
                /* translators: %s: Elementor Pro Forms form ID. */
    ...
    private function submitted_record_field_manifest_metadata_row( string $field_id, mixed $field ): ?array
    {
        ...
        return [
            'label'                   => $label,
            'adminLabel'              => sprintf(
                /* translators: %s: Elementor Pro Forms field id. */
                __( 'Elementor Pro Forms: %s', 'sentient-forms' ),
                $field_id
            ),
            'type'                    => $field_type,
            'visibility'              => 'visible',
