...
final class Sentient_Forms_Plugin
{
    ...
    public function execute_local_action_mapping( array $form, array $entry, array $context ): array | WP_Error
    {
        ...
        $service    = $this->get_local_action_execution_service();
        if ( $mapping_id <= 0 )
        {
            return new WP_Error(
                'sentient_forms_local_mapping_required',
                __( 'This Action mapping predates local Action authority and must be saved again before it can run.', 'sentient-forms' )
            );
        }

        return $service->execute_mapping( $mapping_id, $form, $entry, $context );
    }
    ...
    public function get_telemetry_settings(): array
    {
        ...
        $settings                     = $this->get_telemetry_defaults();
        $settings['telemetry_opt_in'] = ! empty( $stored['telemetry_opt_in'] );
        $settings['updated_at']       = isset( $stored['updated_at'] )
            ? sanitize_text_field( (string) $stored['updated_at'] )
            : null;
    ...
    {
        $options                    = $this->get_options();
        $merged                     = $this->get_telemetry_defaults();
        $merged['telemetry_opt_in'] = ! empty( $settings['telemetry_opt_in'] );
        $merged['updated_at']       = isset( $settings['updated_at'] )
